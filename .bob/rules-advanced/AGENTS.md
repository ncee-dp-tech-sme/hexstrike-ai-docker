...
**Two-Process System (Cannot Merge):**
...
- Full access to MCP and Browser tools in this mode
...
- Chromium + chromium-driver installed in Docker (lines 649-651 in Dockerfile)
- Selenium WebDriver uses isolated Chrome instance
- Headless mode default (can override with `headless=False`)
- Screenshots saved to `/tmp` by default

**MCP Server Access:**
- hexstrike-ai MCP server provides 150+ security tools
- MUST verify server running: `curl http://localhost:8888/health`
- Tool status endpoint: `curl http://localhost:8888/api/tools/status`
